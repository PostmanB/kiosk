...
export const STATS_PIN_UNLOCK_KEY = "kiosk_stats_pin_unlocked_until";
...
const getStoredUnlockUntil = () => {
  ...
  const raw = localStorage.getItem(STATS_PIN_UNLOCK_KEY);
...
const StatsGate = ({ children }: StatsGateProps) => {
  ...
  if (requiredLength < 4 || requiredLength > 8 || /\D/.test(configuredPin)) {
    return (
      <div className="min-h-screen bg-primary text-contrast">
        <div className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center gap-6 px-4 py-12">
          <div className="rounded-3xl border border-accent-3/60 bg-accent-1/80 p-6 shadow-lg shadow-accent-4/20">
  ...
  const normalizePin = (value: string) =>
    value.replace(/\D/g, "").slice(0, requiredLength);
  ...
  const [unlocked, setUnlocked] = useState(() => {
    ...
    if (unlockUntil) {
      localStorage.removeItem(STATS_PIN_UNLOCK_KEY);
    }
  ...
  useEffect(() => {
    ...
    const resetTimer = () => {
      ...
      localStorage.setItem(STATS_PIN_UNLOCK_KEY, String(unlockUntil));
      if (timeoutId) {
        ...
        <div className="rounded-3xl border border-accent-3/60 bg-accent-1/80 p-6 shadow-lg shadow-accent-4/20">
          ...
          <div className="mt-6 space-y-3">
            <input
              ...
              value={pinInput}
              onChange={(event) => setPinInput(normalizePin(event.target.value))}
              onKeyDown={(event) => {
                if (event.key === "Enter") {
                  handleVerify();
                }
